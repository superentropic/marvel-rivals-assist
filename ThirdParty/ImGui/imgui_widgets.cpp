...
// U8 macro for UTF-8 string literals (PUBG-specific, pass-through for Marvel Rivals)
#ifndef U8
#define U8(s) s
#endif
...
bool ImGui::Keybind(const char* label, int* key, bool showlabel)
{
    ...
    ImRect clickable2(ImVec2(clickable.Min.x - 40, clickable.Min.y), ImVec2(clickable.Min.x - 10, clickable.Max.y));

    ItemSize(ImRect(clickable.Min, clickable.Max - ImVec2(0, 2)));
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, strcmp(buf_display, "None") != 0 ? c::text::text_hov : hovered ? c::text::text_hov : c::text::text, ImGui::GetIO().DeltaTime * 6.f);

    window->DrawList->AddRectFilled(ImVec2(clickable2.Min.x, clickable2.Min.y), clickable2.Max, GetColorU32(c::keybind::background), c::keybind::rounding);

    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(c::keybind::background), c::keybind::rounding);
    if (showlabel) RenderTextColor(window->DrawList->_Data->Font, ImVec2(rect.Min + ImVec2(0, 3)), ImVec2(FLT_MAX, FLT_MAX), GetColorU32(it_anim->second.text), label, ImVec2(0, 0));

    GetWindowDrawList()->AddText(font::icomoon_menu, 15.f, clickable2.Min + ImVec2(9, 2), GetColorU32(it_anim->second.background), "G");

    PushStyleColor(ImGuiCol_Text, it_anim->second.background);
    RenderTextClipped(clickable.Min - ImVec2(0, 1), clickable.Max - ImVec2(0, 1), buf_display, NULL, &label_size, ImVec2(0.5f, 0.38f));
    PopStyleColor();
...
bool ImGui::BeginPopupW(const char* id, const char* hint)
{
...
void ImGui::RenderTextColor(ImFont* font, const ImVec2& p_min, const ImVec2& p_max, ImU32 col, const char* text, const ImVec2& align)
{
    PushFont(font);
...
void ImGui::TextColoredWrapped(const char* label, ImU32 color, float wrap_size)
{
...
void ImGui::DrawVerticalHexagon(ImDrawList* draw_list, ImVec2 center, float size, ImU32 color)
{
    ...
    {
        float angle = i * 2 * IM_PI / num_segments + IM_PI / 2;
        points[i] = ImVec2(center.x + size * cos(angle), center.y + size * sin(angle));
    }

    draw_list->AddConvexPolyFilled(points, num_segments, color);
}
void ImGui::TextColoredWishFont(ImFont* font, const ImVec4& col, const char* fmt, ...)
{
    ImGui::PushFont(font);
    va_list args;
    va_start(args, fmt);
...
void ImGui::DrawFoldedRectangle(ImDrawList* draw_list, ImVec2 position, ImVec2 size, ImU32 color, ImU32 trianglecolor, int down ,int arrow) {

    draw_list->AddRectFilled(position, ImVec2(position.x + size.x, position.y + size.y), color);

    ImVec2 left_position = ImVec2(position.x, position.y);

    ImVec2 right_position = ImVec2(position.x + size.x - down, position.y);
    ...
    if (arrow == 0)
    {
        ...
        draw_list->AddTriangleFilled(
            left_position,
            ImVec2(left_position.x + down, left_position.y),
            ImVec2(left_position.x, left_position.y + down),
            trianglecolor
        );
    }
    else if (arrow == 1)
    {
        ...
        draw_list->AddTriangleFilled(
            right_position,
            ImVec2(right_position.x + down, right_position.y),
            ImVec2(right_position.x + down, right_position.y + down),
            trianglecolor
        );
    }
    else if (arrow == 2)
...
struct tab_state
{
    ImVec4 background, shadow, line, text, icon_color, bg, mark;
};
...
bool ImGui::Tabs(bool selected, const char* icon, const char* label)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    ImVec2 size = CalcItemSize(label_size + ImVec2(icon_size.x + 10, icon_size.y), label_size.x + icon_size.x + 10, label_size.y + icon_size.y);
    ...
    it_anim->second.bg = ImLerp(it_anim->second.bg, selected ? ImColor(c::accent) : ImColor(0, 0, 0, 0), 1);

    GetWindowDrawList()->AddText(font::icomoon_menu, 17, bb.Min + ImVec2(0, 0), GetColorU32(it_anim->second.text), icon);

    PushFont(font::calibri_bold);
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderText(bb.Min + ImVec2(icon_size.x + 10, 2), label);
    PopStyleColor();
    PopFont();
...
bool ImGui::SubTabs(ImFont* font, bool selected, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.bg = ImLerp(it_anim->second.bg, selected ? ImColor( c::accent) : ImColor(0,0,0,0), g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y-2), bb.Max, GetColorU32(it_anim->second.bg));

    PushFont(font);
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderText(bb.Min + (size - label_size) * 0.5f - ImVec2(0,3), label);
    PopStyleColor();
    PopFont();
...
struct button_state
{
    ImVec4 text, icon_color, background, mark;
    float shadow;
};
...
bool ImGui::ButtonEx1(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.shadow = ImLerp(it_anim->second.shadow, IsItemActive() ? 125.f : 0.f, g.IO.DeltaTime * 5.f);

    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::accent1), c::button::rounding1);

    ImGui::GetWindowDrawList()->AddShadowRect(bb.Min + ImVec2(10, 10), bb.Max - ImVec2(10, 10), GetColorU32(c::accent1), it_anim->second.shadow, ImVec2(0.f, 0.f), c::button::rounding1);

    PushFont(font_inter::inter_bold);
    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), GetColorU32(c::text::text_active1), label);
    PopFont();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags,bool showbg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::accent : hovered ? c::button::outline : c::button::background, g.IO.DeltaTime * 6.f);

    if(showbg) GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), c::button::rounding);

    PushStyleColor(ImGuiCol_Text, c::text::text_active);

    RenderText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2 - 4), label);
...
bool ImGui::NewButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, const char* icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map<ImGuiID, Newbutton_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, Newbutton_state() });
        it_anim = anim.find(id);
    }
    ...
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, hovered ? theme_a::main_color : theme_a::background_color, anim_speed);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.frame_color), style.FrameRounding);

    window->DrawList->AddText(
        ImVec2(
            center_text(bb.Min, bb.Max, label).x + icon_width / 2, // 文本向右偏移
            center_text(bb.Min, bb.Max, label).y
        ),
    ...
    for (int i = 0; i < 50; i++)
    {
        ...
        it_anim->second.particle_color[i] = ImLerp(it_anim->second.particle_color[i], ImColor(theme_a::main_color.Value.x, theme_a::main_color.Value.y, theme_a::main_color.Value.z, (it_anim->second.particle_pos[i].y - bb.Max.y)), anim_speed);
    ...
    window->DrawList->AddText(
        ImVec2(
            center_text(bb.Min, bb.Max, label).x - CalcTextSize(label).x / 2 - icon_width - 5.0f, // 图标位于文本左侧，间距5.0f
            center_text(bb.Min, bb.Max, icon).y
        ),
        theme_a::text_color[0],
        icon
    );
...
// #include <Utils\Utils.h> // PUBG-specific, not needed for Marvel Rivals
...
bool ImGui::Product(const char* label, ImTextureID texture_id, int number, const char* subscribe, const char* last_update, const char* last_launch)
{
    ...
    // 保持原宽度700，增加高度到280
    const ImRect bb(pos, pos + ImVec2(700, 280));

    // 启动按钮位置向下移动到180
    const ImRect button_bb(bb.Min + ImVec2(370, 180), bb.Min + ImVec2(370, 180) + ImVec2(310, 50));

    ItemSize(ImVec2(700, 280), style.FramePadding.y);
    ItemAdd(bb, id);
    ...
    // 绘制主背景
    window->DrawList->AddRectFilled(bb.Min, bb.Max, theme_a::background_color, style.FrameRounding);

    // 绘制启动按钮
    window->DrawList->AddRectFilled(button_bb.Min, button_bb.Max, theme_a::main_color, style.FrameRounding);
    window->DrawList->AddText(center_text(ImVec2(button_bb.Min.x - icon_width / 2, button_bb.Min.y), button_bb.Max, U8("启动  ")), theme_a::text_color[0], U8("启动  "));

    PushFont(icon_small);
    icon_width = CalcTextSize("l").x;
    window->DrawList->AddText(ImVec2(center_text(ImVec2(button_bb.Min.x - icon_width / 2, button_bb.Min.y), button_bb.Max, U8("启动  ")).x + CalcTextSize(U8("启动  ")).x - CalcTextSize("l").x / 4, center_text(button_bb.Min, button_bb.Max, "l").y), theme_a::text_color[0], "l");
    PopFont();

    // 左侧图片区域：保持宽度350，增加高度到280
    window->DrawList->AddImageRounded(texture_id, bb.Min, bb.Min + ImVec2(350, 280), ImVec2(0, 0), ImVec2(1, 1), ImColor(1.f, 1.f, 1.f, 1.f), style.FrameRounding, ImDrawFlags_RoundCornersLeft);

    // 调整图标位置
    PushFont(big_icon);
    window->DrawList->AddText(center_text(bb.Min, bb.Min + ImVec2(350, 280), product_icons[number]) - ImVec2(0, 25), theme_a::text_color[0], product_icons[number]);
    PopFont();

    // 重新排列右侧信息，将三个信息项对齐成一列，增加间距
    PushFont(medium_icon_font);
    window->DrawList->AddText(bb.Min + ImVec2(383, 30) - ImVec2(7, 6), theme_a::main_color, "5");
    window->DrawList->AddText(bb.Min + ImVec2(383, 80) - ImVec2(7, 6), theme_a::main_color, "3");
    window->DrawList->AddText(bb.Min + ImVec2(383, 130) - ImVec2(7, 6), theme_a::main_color, "4");
    PopFont();

    // 将三个文本标签对齐成一列，增加间距
    window->DrawList->AddText(bb.Min + ImVec2(410, 22), theme_a::text_color[0], U8("卡密剩余时间:"));
    window->DrawList->AddText(bb.Min + ImVec2(410, 72), theme_a::text_color[0], U8("最后更新:"));
    window->DrawList->AddText(bb.Min + ImVec2(410, 122), theme_a::text_color[0], U8("最后启动:"));

    // 将对应的数据也对齐成一列，增加间距
    PushFont(font::calibri_bold); // 替换 your_specific_font 为您想使用的字体变量
    window->DrawList->AddText(bb.Min + ImVec2(410, 42), theme_a::main_color, subscribe);
    PopFont();

    window->DrawList->AddText(bb.Min + ImVec2(410, 92), theme_a::main_color, last_update);
    window->DrawList->AddText(bb.Min + ImVec2(410, 142), theme_a::main_color, last_launch);

    // 调整标签位置，适应新的图片高度
    window->DrawList->AddText(center_text(bb.Min, bb.Min + ImVec2(350, 280), label) + ImVec2(0, 25), theme_a::text_color[0], label);

    // 调整渐变遮罩，使用新的高度
    window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Min + ImVec2(350, 280), ImColor(theme_a::background_color.Value.x, theme_a::background_color.Value.y, theme_a::background_color.Value.z, 0.f), theme_a::background_color, theme_a::background_color, ImColor(theme_a::background_color.Value.x, theme_a::background_color.Value.y, theme_a::background_color.Value.z, 0.f));

    return pressed;
}

bool ImGui::NewButton(const char* label, const ImVec2& size_arg, const char* icon)
{
    return NewButtonEx(label, size_arg, ImGuiButtonFlags_None, icon);
}


bool ImGui::Button(const char* label, const ImVec2& size_arg, bool showbg)
{
    ...
    std::string searchWidgetsLower = search_all_widgets_;
    std::transform(searchWidgetsLower.begin(), searchWidgetsLower.end(), searchWidgetsLower.begin(),
        [](unsigned char c) { return std::tolower(c); });

    if (!searchWidgetsLower.empty() && labelstr.find(searchWidgetsLower) == std::string::npos) {
        return false;
    }

    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, showbg);
}

bool ImGui::Button_iconEx(const char* icon, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    RenderText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2 - CalcTextSize(icon).x-2, bb.Max.y - CalcTextSize(icon).y - (size.y - CalcTextSize(icon).y) / 2-2), icon);
    
    PopFont();

    RenderText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2 + CalcTextSize(icon).x+2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2 - 4), label);
...
bool ImGui::Button_icon(const char* icon, const char* label, const ImVec2& size_arg)
{
    std::string labelstr = label;
    ...
    return Button_iconEx(icon,label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::color_button(const char* name, ImVec2 size, ImU32 color_bg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(color_bg), held ? 12.f : 4.f);
...
void ImGui::icon_box(ImTextureID id, ImVec2 size, ImU32 color_icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos + size);
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::input::background), c::input::rounding);
    ImGui::GetWindowDrawList()->AddRect(rect.Min, rect.Max, GetColorU32(c::input::outline), c::input::rounding);

    ImGui::GetWindowDrawList()->AddImage(id, rect.Min + ImVec2(5, 5), rect.Max - ImVec2(5, 5), ImVec2(0, 0), ImVec2(1, 1), color_icon);

}

bool ImGui::Tab1(bool selectable, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    if (it_anim == anim.end())
    {
        anim.insert(std::make_pair(id, tab_state()));
        it_anim = anim.find(id);
    }

    it_anim->second.background = ImLerp(it_anim->second.background, selectable ? c::accent1 : c::accent_transparent, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selectable ? c::accent1 : c::text::text1, g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(size_arg.x / 4, size_arg.y - 5), bb.Max - ImVec2(size_arg.x / 4, 0), GetColorU32(it_anim->second.background), 30.f, ImDrawCornerFlags_Top);
    ImGui::GetWindowDrawList()->AddShadowRect(bb.Min + ImVec2(size_arg.x / 4, size_arg.y - 5), bb.Max - ImVec2(size_arg.x / 4, 0), GetColorU32(it_anim->second.background), 60.f, ImVec2(0.f, 0.f), 30.f, ImDrawCornerFlags_Top);
...
bool ImGui::color_butto2n(const char* name, ImVec2 size, ImU32 color_bg, int sp)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    ItemSize(ImVec2(size.x, sp == 0 ? 6 : size.y), 0.f);
    if (!ItemAdd(rect, id)) return false;
    ...
    window->DrawList->AddShadowRect(!hovered ? rect.Min : rect.Min - ImVec2(3, 3), !hovered ? rect.Max : rect.Max + ImVec2(3, 3), GetColorU32(c::shadow, 0.8f), 10, ImVec2(0, 0));
    window->DrawList->AddRectFilled(!hovered ? rect.Min : rect.Min - ImVec2(3, 3), !hovered ? rect.Max : rect.Max + ImVec2(3, 3), GetColorU32(color_bg));
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, held ? c::accent : hovered ? c::scrollbar::bar_hov : c::scrollbar::bar, g.IO.DeltaTime * 6.f);
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y - 2, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y - 20, grab_v_norm) + grab_h_pixels);

    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(c::accent), 0.f);
...
struct check_state
{
    std::vector<std::string> pageTexts = { "Copy","Description","New KeyBind","Main Color","Child Color"};
    std::vector<std::string> icons = {"a", "r", "u", "e", "e" };

    std::vector<std::string> hitbox_page = { "Copy","Sequence"};
    std::vector<std::string> hitbox_icons = { "a", "r" };

    std::vector<std::string> itembox_page = { "Copy","Item Color" };
    std::vector<std::string> itembox_icons = { "a", "e" };

    ImVec4 background, circle, text, mark, image;
    float mark_alpha, background_opticaly, circle_offset, popup_opticaly, alpha, move, size, sizeve, muitalpha, mark_pos;
    bool active, hovered;
    int page_selector = 0;
...
bool ImGui::Hive_Checkbox(ImTextureID image, const char* label, bool v, const ImVec2& size_arg, float sizeve, const ImVec2& mousePos, const ImVec2& labelPos)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(size_arg.x, size_arg.y));
    ItemSize(total_bb, 0.f);

    if (!ItemAdd(total_bb, id)) return false;

    bool held, hovered, pressed = ButtonBehavior(total_bb, id, &hovered, &held);
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, v ? c::accent : hovered ? c::accent : c::checkbox::background, g.IO.DeltaTime * 6.f);
    ...
    it_anim->second.move = ImLerp(it_anim->second.move, v ? 8.f : hovered ? 4.f : 0.f, g.IO.DeltaTime * 6.f);

    it_anim->second.size = ImLerp(it_anim->second.size, (v && hovered && ImGui::IsMousePosValid() && ImGui::IsMouseHoveringRect(total_bb.Min, total_bb.Max, false)) ? 40.f : (v ? 40.f : hovered ? 20.f : 40.f), g.IO.DeltaTime * 10.f);

    float distance = sqrt(pow(mousePos.x - (total_bb.Min.x + total_bb.Max.x) / 2, 2) + pow(mousePos.y - (total_bb.Min.y + total_bb.Max.y) / 2, 2));

    if (distance < 110)
    {
        it_anim->second.size = ImLerp(it_anim->second.size, !v && !hovered ? 20.f : v ? 40.f : 40.f, g.IO.DeltaTime * 10.f);
    }

    it_anim->second.sizeve = ImLerp(it_anim->second.sizeve, v && hovered ? 35.f : v ? 45.f : hovered ? 45.f : 40.f, g.IO.DeltaTime * 10.f);
    it_anim->second.alpha = ImLerp(it_anim->second.alpha, v && hovered ? 1.f : v ? 0.f : hovered ? 1.f : 0.f, g.IO.DeltaTime * 10.f);
    it_anim->second.mark_pos = ImLerp(it_anim->second.mark_pos, v && hovered ? 3.f : v ? 0.f : hovered ? 3.f : 0.f, g.IO.DeltaTime * 10.f);


    const ImRect check_bb(pos, pos + ImVec2(size_arg.x, size_arg.y));


    ImVec2 shadow_pos = ImVec2(check_bb.Min.x + size_arg.x / 2, check_bb.Min.y + size_arg.y / 2);
    GetWindowDrawList()->AddShadowCircle(shadow_pos, 12, GetColorU32(c::shadow, 0.6f), 88, ImVec2(0, 0));

    DrawVerticalHexagon(GetWindowDrawList(), ImVec2(check_bb.Min.x + size_arg.x / 2, check_bb.Min.y + size_arg.x / 2), it_anim->second.size, GetColorU32(it_anim->second.background));

    ImVec2 image_pos = ImVec2(check_bb.Min.x + (size_arg.x - it_anim->second.sizeve) / 2, check_bb.Min.y + (size_arg.y - it_anim->second.sizeve) / 2);
    GetWindowDrawList()->AddImage(image, image_pos, image_pos + ImVec2(it_anim->second.sizeve, it_anim->second.sizeve));

    if (hovered)
    {
        DrawFoldedRectangle(GetForegroundDrawList(), labelPos - ImVec2(5, 2), label_size + ImVec2(5, 10), GetColorU32(c::accent), GetColorU32(c::child::background), 6, 0);

        GetForegroundDrawList()->AddText(labelPos - ImVec2(1, 0), GetColorU32(c::bg::background), label);
    }

    return pressed;
}

bool ImGui::Hive_Checkbox_Radio(ImTextureID image, const char* label, int* v, int v_button, const ImVec2& size_arg, float sizeve, const ImVec2& mousePos, const ImVec2& labelPos)
{
    const bool pressed = Hive_Checkbox(image, label, *v == v_button, size_arg, sizeve, mousePos, labelPos);
    if (pressed)
        *v = v_button;
    return pressed;
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = 18;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + w, square_sz));
    ItemSize(total_bb, 0.f);
    ...
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? c::accent : c::checkbox::background, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.move = ImLerp(it_anim->second.move, *v ? -14.f : 0.f, 1);
    it_anim->second.alpha = ImLerp(it_anim->second.alpha, *v ? 1.f : 0.f, g.IO.DeltaTime * 6.f);

    const ImRect check_bb(pos + ImVec2(3, 0), pos + ImVec2(square_sz - 3, square_sz));


    GetWindowDrawList()->AddShadowRect(check_bb.Min + ImVec2((w - 20), 0), check_bb.Max + ImVec2(w, 0), GetColorU32(c::shadow, 0.4f), 10, ImVec2(0, 0));

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2((w - 20), 0), check_bb.Max + ImVec2(w, 0), GetColorU32(it_anim->second.circle), c::checkbox::rounding);
    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2((w - 16 - it_anim->second.move), +3), check_bb.Max + ImVec2(w - 16 - it_anim->second.move, -3), GetColorU32(c::bg::background), c::checkbox::rounding);

    if (*v) GetWindowDrawList()->AddShadowRect(check_bb.Min + ImVec2((w - 16 - it_anim->second.move), +3), check_bb.Max + ImVec2(w - 16 - it_anim->second.move, -3), GetColorU32(c::shadow, it_anim->second.alpha), 10.f, ImVec2(0, 0), 30);


    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x - square_sz + 3, check_bb.Min.y + 3), GetColorU32(it_anim->second.text), label);
...
bool ImGui::Checkbox1(const char* label, bool* v)
{
    ...
    it_anim->second.background_offset = ImLerp(it_anim->second.background_offset, *v ? ImVec2(0, 0) : ImVec2(2, 2), g.IO.DeltaTime * 6.f);

    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::accent1 : c::checkbox1::background, g.IO.DeltaTime * 6.f);
    it_anim->second.mark = ImLerp(it_anim->second.mark, *v ? c::checkbox1::checkmark_active : c::checkbox1::checkmark_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.mark_animation = ImLerp(it_anim->second.mark_animation, *v ? 0.f : square_sz, g.IO.DeltaTime * 3.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? c::text::text_active1 : c::text::text1, g.IO.DeltaTime * 6.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background), c::checkbox1::rounding);
...
bool ImGui::Checkbox_hitbox(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, *v && hovered ? c::text::text_active : *v ? c::bg::background : hovered ? c::text::text_hov : c::text::text_active, g.IO.DeltaTime * 6.f);
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::accent : c::child::background, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddShadowRect(check_bb.Min, check_bb.Max, ImColor(7,9,25), 30, ImVec2(0, 0), NULL, 100);
    GetWindowDrawList()->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background), 100);

    ImVec2 text_size = CalcTextSize("+", NULL, true);
    ImVec2 text_pos = check_bb.Min + (check_bb.GetSize() - text_size) * 0.5f;

    GetWindowDrawList()->AddText(text_pos-ImVec2(0,1), GetColorU32(it_anim->second.text), "+");
...
void ImGui::Pickerbox(const char* label, bool* v, float color[3], ImGuiColorEditFlags flags)
{
    ...
    ImGui::SetCursorPos(GetCursorPos() - ImVec2(45, 0));
    ImGui::ColorEdit4(label_bind.c_str(), color, flags | ImGuiColorEditFlags_NoLabel);
    ImGui::SetCursorPos(GetCursorPos() - ImVec2(0, 32));
    ImGui::Checkbox(label, v);

}

void ImGui::Pickerbox2(const char* label, bool* v, float color[4], int colCount, float colors[][4], ImGuiColorEditFlags flags)
{
    std::string label_key = "picker", label_bind = label_key + label;
    ImGui::ColorEdit4(label_bind.c_str(), color, flags | ImGuiColorEditFlags_NoLabel);
    ...
    for (int i = 0; i < colCount; ++i)
    {
        ...
        ImGui::SetCursorPos(GetCursorPos() - ImVec2(offsetX * (i + 1), 32));
        std::string label_bind_dynamic = "picker_" + std::to_string(i + 2) + label;
        ImGui::ColorEdit4(label_bind_dynamic.c_str(), colors[i], flags | ImGuiColorEditFlags_NoLabel);
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    state.background = ImLerp(state.background, active ? c::accent : c::checkbox::background, g.IO.DeltaTime * 12.f);
    state.circle = ImLerp(state.circle, active ? c::accent : c::checkbox::mark, g.IO.DeltaTime * 12.f);
    state.text = ImLerp(state.text, active ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 12.f);
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(state.background));
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(c::checkbox::outline));


    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y-1);
    if (g.LogEnabled)
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    RenderRectFilledRangeH(window->DrawList, bb, ImColor(252, 206, 7), 0.0f, fraction, style.FrameRounding);
    char overlay_buf[32];
    if (!overlay)
    {
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        const ImRect bb(ImVec2(window->DC.CursorPos.x+15, y1), ImVec2(window->DC.CursorPos.x + thickness-15, y2));
        ItemSize(ImVec2(thickness, 0.0f));
        if (!ItemAdd(bb, 0))
            return;

        // Draw
        window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::text::text,0.2f));
        if (g.LogEnabled)
            LogText(" |");
    }
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        const ImRect bb(ImVec2(x1+15, window->DC.CursorPos.y), ImVec2(x2-15, window->DC.CursorPos.y + thickness));
...
bool ImGui::colorpicker_button(ImFont* font, const char* tips, const char* label, const ImVec2& size_arg, bool* callback)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(tips);
    ...
    bool hovered = ItemHoverable(rect, id, g.LastItemData.InFlags);
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, *callback ? c::accent : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);


    window->DrawList->AddShadowRect(!hovered ? rect.Min : rect.Min - ImVec2(3, 3), !hovered ? rect.Max : rect.Max + ImVec2(3, 3), GetColorU32(c::shadow, 0.8f), 10, ImVec2(0, 0));
    window->DrawList->AddRectFilledMultiColor(!hovered ? rect.Min : rect.Min - ImVec2(3, 3), !hovered ? rect.Max : rect.Max + ImVec2(3, 3), ImColor(255, 0, 0, 255), ImColor(0, 255, 0, 255), ImColor(0, 0, 255, 255), ImColor(255, 255, 0, 255));


    if (hovered)
    {

        ImVec2 rectMin(rect.Min.x + (size_arg.x - CalcTextSize(tips).x) / 2 - 5, rect.Max.y - CalcTextSize(tips).y - (size.y - CalcTextSize(tips).y) / 2 - 35);
        ImVec2 rectMax(rectMin.x + CalcTextSize(tips).x + 10, rectMin.y + CalcTextSize(tips).y + 10);

        GetForegroundDrawList()->AddShadowRect(rectMin, rectMax, GetColorU32(c::shadow), 10, ImVec2(0, 0));

        GetForegroundDrawList()->AddText(ImVec2(rectMin.x + 5, rectMin.y + 5), GetColorU32(c::text::text_active), tips);
    }
...
bool ImGui::SliderInt1(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags, bool search_enable)
{
    if (search_enable)
    {
        std::string labelstr = label;
...
bool ImGui::Item_checkbox(const ImVec2& image_size, ImTextureID image, const char* label, bool* v, const ImVec2& size_arg, int lv)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (lv == 0)
        ...
        color = ImColor(17, 232, 28);
    else if (lv == 2)
        color = ImColor(88, 127, 205);
    else if (lv == 3)
        color = ImColor(139, 110, 200);
    else if (lv == 4)
        color = ImColor(255, 204, 28);
    else if (lv == 5)
        color = ImColor(245, 17, 23);

    it_anim->second.background = ImLerp(it_anim->second.background, *v ? ImColor(color) : hovered ? ImColor(color) : ImColor(color.Value.x, color.Value.y, color.Value.z, color.Value.w / 2), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.move = ImLerp(it_anim->second.move, *v ? 8.f : hovered ? 4.f : 0.f, g.IO.DeltaTime * 6.f);
    it_anim->second.size = ImLerp(it_anim->second.size, *v ? 4.f : hovered ? -5.f : 0.f, g.IO.DeltaTime * 6.f);
    it_anim->second.sizeve = ImLerp(it_anim->second.sizeve, *v ? 6.f : hovered ? 6.f : 0.f, g.IO.DeltaTime * 6.f);

    it_anim->second.alpha = ImLerp(it_anim->second.alpha, *v ? 1.0f : hovered ? 0.4f : 0.0f, g.IO.DeltaTime * 6.f);
    it_anim->second.muitalpha = ImLerp(it_anim->second.muitalpha, *v ? 0.45f : hovered ? 0.45f : 0.3f, g.IO.DeltaTime * 6.f);


    const ImRect check_bb(pos, pos + ImVec2(size_arg.x, size_arg.y));

    GetWindowDrawList()->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.background, 0.6f), GetColorU32(it_anim->second.background, 0.f), GetColorU32(it_anim->second.background, 0.4f));

    ImVec2 image_pos_ = ImVec2(check_bb.Min.x + (size_arg.x - (image_size.x + it_anim->second.sizeve)) / 2, check_bb.Min.y + (size_arg.y - (image_size.y + (it_anim->second.sizeve / 2))) / 2);
    ImVec2 image_size_ = ImVec2(image_size.x + it_anim->second.sizeve, image_size.y + (it_anim->second.sizeve / 2));

    GetWindowDrawList()->AddImage(image, image_pos_, ImVec2(image_pos_.x + image_size_.x, image_pos_.y + image_size_.y), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::image));

    GetWindowDrawList()->AddText(font::calibri_bold, 14, ImVec2(check_bb.Min.x + 5, check_bb.Max.y - 20), GetColorU32(it_anim->second.text), label);

    GetWindowDrawList()->AddRect(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background, it_anim->second.alpha));
...
int rotation_start_index_;
void ImRotateStart_()
{
    rotation_start_index_ = ImGui::GetWindowDrawList()->VtxBuffer.Size;
}

ImVec2 ImRotationCenter_()
{
    ImVec2 l(FLT_MAX, FLT_MAX), u(-FLT_MAX, -FLT_MAX);

    const auto& buf = ImGui::GetWindowDrawList()->VtxBuffer;
    for (int i = rotation_start_index_; i < buf.Size; i++)
        l = ImMin(l, buf[i].pos), u = ImMax(u, buf[i].pos);

    return ImVec2((l.x + u.x) / 2, (l.y + u.y) / 2);
}

void ImRotateEnd_(float rad, ImVec2 center = ImRotationCenter_())
{
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
{
    ...
    // 控制选择框的宽度，可以根据需要调整
    float combo_width = w * 0.25f; // 调整为总宽度的 75%

    const ImRect bb(window->DC.CursorPos + ImVec2(0, 25), window->DC.CursorPos + ImVec2(combo_width, 55));
    ...
    it_anim->second.alpha = ImLerp(it_anim->second.alpha, it_anim->second.opened_combo ? 1.0f : hovered ? 1.f : 0.8f, g.IO.DeltaTime * 6.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - GetFrameHeight());

    // 控制背景缩短的宽度（水平长度）
    float background_width_reduction = 580.0f;  // 可根据需要调整这个值
    ImVec2 reduced_bb_max = ImVec2(bb.Max.x - background_width_reduction, bb.Max.y); // 减少背景的水平长度

    // 绘制缩短后的背景，修改为黑色背景
    GetWindowDrawList()->AddRectFilled(bb.Min, reduced_bb_max, 6, ImDrawFlags_RoundCornersBottomRight);

    RenderTextColor(window->DrawList->_Data->Font, ImVec2(bb.Max.x - w, bb.Min.y - 23), ImVec2(FLT_MAX, FLT_MAX), GetColorU32(it_anim->second.text), label, ImVec2(0, 0));

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2((30 - CalcTextSize(preview_value).y), (25 - CalcTextSize(preview_value).y)) / 2, ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();

    ImRotateStart_();
    GetWindowDrawList()->AddText(font::icomoon_menu, 17, ImVec2(bb.Max.x - (50 + CalcTextSize("l").y) / 2, bb.Max.y - (40 + CalcTextSize("l").y) / 2), GetColorU32(it_anim->second.text), "l");
    ImRotateEnd_(1.57f * it_anim->second.arrow_roll);
...
bool ImGui::BeginCombo_popup(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
{
    std::string labelstr = label;
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min, ImVec2(bb.Max.x, bb.Max.y), GetColorU32(c::combo::background), c::combo::rounding);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_Border, GetColorU32(c::combo::outline));
    PushStyleVar(ImGuiStyleVar_WindowRounding,12);
...
bool ImGui::Combo_popup(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginCombo_popup(label, preview_value, items_count, ImGuiComboFlags_None))
        return false;
...
bool ImGui::Combo_popup(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items)
{
    const bool value_changed = Combo_popup(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items);
    return value_changed;
}

bool ImGui::Combo_popup(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    int items_count = 0;
    const char* p = items_separated_by_zeros;       // FIXME-OPT: Avoid computing this, or at least only when combo is open
    while (*p)
    ...
    bool value_changed = Combo_popup(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}
...
struct slider_state {
    ImVec4 background, circle, text, mark, mark2;
    float slow_anim, circle_anim,size;
};
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(23, label_size.y + 14), window->DC.CursorPos + ImVec2(w - 23, label_size.y + 29.f));
    ...
    it_anim->second.circle_anim = ImLerp(it_anim->second.circle_anim, IsItemActive() ? 14.f : 12.f, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 3), frame_bb.Max - ImVec2(0, 3), GetColorU32(c::slider::background), 0.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 3), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y - 2), GetColorU32(c::accent), 0.f);
        GetWindowDrawList()->AddShadowRect(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 5, grab_bb.Min.y - 4), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 15, grab_bb.Max.y + 2), GetColorU32(c::shadow), 10, ImVec2(0, 0));
        GetWindowDrawList()->AddRectFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 5, grab_bb.Min.y - 4), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 15, grab_bb.Max.y + 2), GetColorU32(c::accent));
    }

    it_anim->second.mark = ImLerp(it_anim->second.mark, IsMouseHoveringRect(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), true) ? c::text::text_active : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.mark2 = ImLerp(it_anim->second.mark2, IsMouseHoveringRect(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), true) ? c::text::text_active : c::text::text, g.IO.DeltaTime * 6.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.mark));
    RenderTextClipped(frame_bb.Max - ImVec2(-8, 20), frame_bb.Max + ImVec2(23, 0), "+", NULL, NULL, ImVec2(0.5, 0.45), NULL);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.mark2));
    RenderTextClipped(frame_bb.Min - ImVec2(23, 4), frame_bb.Min + ImVec2(-8, 15), "-", NULL, NULL, ImVec2(0.5, 0.32), NULL);
    PopStyleColor(2);

    // 类型感知的增减逻辑
    const float step_float = 0.1f;
    const int step_int = 1;

    if (IsMouseHoveringRect(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), true) && GetIO().MouseClicked[0]) {
        if (data_type == ImGuiDataType_Float) (*(float*)p_data) += step_float;
        else if (data_type == ImGuiDataType_S32) (*(int*)p_data) += step_int;
    }

    if (IsMouseHoveringRect(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), true) && GetIO().MouseClicked[0]) {
        if (data_type == ImGuiDataType_Float) (*(float*)p_data) -= step_float;
        else if (data_type == ImGuiDataType_S32) (*(int*)p_data) -= step_int;
    }

    static float hold_time_incr = 0.0f;
    static float hold_time_decr = 0.0f;

    if (IsMouseHoveringRect(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), true)) {
        if (GetIO().MouseDown[0]) {
            hold_time_incr += g.IO.DeltaTime;
            if (hold_time_incr > 0.5f) {
                if (data_type == ImGuiDataType_Float) (*(float*)p_data) += step_float * (1 + (hold_time_incr - 0.5f) * 1.1f);
                else if (data_type == ImGuiDataType_S32) (*(int*)p_data) += step_int + (int)((hold_time_incr - 0.5f) * 1.1f);
            }
        }
        else {
            hold_time_incr = 0.0f;
    ...
    if (IsMouseHoveringRect(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), true)) {
        if (GetIO().MouseDown[0]) {
            hold_time_decr += g.IO.DeltaTime;
            if (hold_time_decr > 0.5f) {
                if (data_type == ImGuiDataType_Float) (*(float*)p_data) -= step_float * (1 + (hold_time_decr - 0.5f) * 1.1f);
                else if (data_type == ImGuiDataType_S32) (*(int*)p_data) -= step_int + (int)((hold_time_decr - 0.5f) * 1.1f);
            }
        }
        else {
            hold_time_decr = 0.0f;
    ...
    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x + 23, frame_bb.Min.y - 28), GetColorU32(c::text::text_active), value_buf);

    RenderTextColor(font::calibri_bold, ImVec2(frame_bb.Max.x - (w - 23), frame_bb.Min.y - 60), frame_bb.Max, GetColorU32(it_anim->second.text), label, ImVec2(0.0, 0.5f));
...
bool ImGui::SliderScalarF(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 3), frame_bb.Max - ImVec2(0, 4), GetColorU32(c::slider::background), c::slider::rounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        it_anim->second.slow_anim = ImLerp(it_anim->second.slow_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 25.f);
        it_anim->second.size = ImLerp(it_anim->second.size, IsItemActive() ? 5.f : hovered ? 5.f : 4.f, g.IO.DeltaTime * 6.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 3), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y-3), GetColorU32(c::accent),  c::slider::rounding);
        GetWindowDrawList()->AddShadowCircle(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Min.y + 5), 8, GetColorU32(c::shadow), 10, ImVec2(0, 0));
        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x+5, grab_bb.Min.y+5), 8, GetColorU32(c::accent), 99);
        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Min.y + 5), it_anim->second.size, GetColorU32(c::child::background), 99);
    ...
    if (IsMouseHoveringRect(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), true) && GetIO().MouseClicked[0]) *(float*)p_data += 1;

    if (IsMouseHoveringRect(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), true) && GetIO().MouseClicked[0]) *(float*)p_data -= 1;
    ...
    if (IsMouseHoveringRect(frame_bb.Max - ImVec2(-8, 15), frame_bb.Max + ImVec2(23, 0), true)) {
        if (GetIO().MouseDown[0]) {
            hold_time_incr += g.IO.DeltaTime;
            if (hold_time_incr > 0.5f) {
                int increment = 1 + static_cast<int>((hold_time_incr - 0.5f) * 1.1);
                *(float*)p_data += increment;
            }
        }
        else {
            hold_time_incr = 0.0f;
    ...
    if (IsMouseHoveringRect(frame_bb.Min - ImVec2(23, 0), frame_bb.Min + ImVec2(-8, 15), true)) {
        if (GetIO().MouseDown[0]) {
            ...
            if (hold_time_decr > 0.5f) {
                int decrement = 1 + static_cast<int>((hold_time_decr - 0.5f) * 1.1);
                *(float*)p_data -= decrement;
            }
        }
        else {
            hold_time_decr = 0.0f;
...
bool ImGui::SliderScalar_popup(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddShadowRect(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 5, grab_bb.Min.y - 4), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 15, grab_bb.Max.y + 2), GetColorU32(c::shadow), 10, ImVec2(0, 0));
        GetWindowDrawList()->AddRectFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 5, grab_bb.Min.y - 4), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 15, grab_bb.Max.y + 2), GetColorU32(c::accent));
        GetWindowDrawList()->AddRectFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 3, grab_bb.Min.y - 2), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 13, grab_bb.Max.y), GetColorU32(c::bg::background));
    }

    PushStyleColor(ImGuiCol_Text, GetColorU32(c::text::text_active));
    RenderTextClipped(frame_bb.Max - ImVec2(-8, 20), frame_bb.Max + ImVec2(23, 0), "+", NULL, NULL, ImVec2(0.5, 0.35), NULL);
    RenderTextClipped(frame_bb.Min - ImVec2(23, 4), frame_bb.Min + ImVec2(-8, 15), "-", NULL, NULL, ImVec2(0.5, 0.32), NULL);
    PopStyleColor();
...
bool ImGui::SliderScalarF_popup(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderFloat_popup(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    std::string labelstr = label;
    ...
    return SliderScalarF_popup(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}

bool ImGui::SliderInt_popup(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    std::string labelstr = label;
    ...
    return SliderScalar_popup(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
static bool InputTextFilterCharacter1(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    IM_ASSERT(input_source == ImGuiInputSource_Keyboard || input_source == ImGuiInputSource_Clipboard);
    unsigned int c = *p_char;
...
static bool NewInputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    IM_ASSERT(input_source == ImGuiInputSource_Keyboard || input_source == ImGuiInputSource_Clipboard);
    unsigned int c = *p_char;
...
struct input_state
{
    ImVec4 text, outline, outline_color;
    float slow, outline_offset, outline_size;
};

bool ImGui::InputTextEx1(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx1(label, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
    ...
    const bool hovered = ItemHoverable1(frame_bb, id);
    if (hovered)
    ...
    if (g.ActiveId == id)
    {
        ...
        if ((flags & ImGuiInputTextFlags_AllowTabInput) && Shortcut(ImGuiKey_Tab, id) && !is_readonly)
        {
            ...
            if (InputTextFilterCharacter1(&c, flags, callback, callback_user_data, ImGuiInputSource_Keyboard))
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                for (const char* s = clipboard; *s != 0; )
                {
                    ...
                    if (!InputTextFilterCharacter1(&c, flags, callback, callback_user_data, ImGuiInputSource_Clipboard))
                        continue;
    ...
    it_anim->second.outline_offset = ImLerp(it_anim->second.outline_offset, IsItemActive() ? size_arg.y - 3 : 0, g.IO.DeltaTime * 16.f);
    it_anim->second.outline_color = ImLerp(it_anim->second.outline_color, IsItemActive() ? c::accent1 : c::input::outline1, g.IO.DeltaTime * 6.f);

    it_anim->second.outline_size = ImLerp(it_anim->second.outline_size, it_anim->second.outline_offset <= (size_arg.y - 7) ? 1.0f : 2.0f, g.IO.DeltaTime * 6.f);

    if (!is_multiline)
    {

        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::input::background1), c::input::rounding1);

        PushClipRect(frame_bb.Min + ImVec2(0, it_anim->second.outline_offset), frame_bb.Max, true);
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max - ImVec2(0, it_anim->second.outline_size), GetColorU32(it_anim->second.outline_color), c::input::rounding1, NULL, it_anim->second.outline_size);
        PopClipRect();
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? c::text::text1 : c::text::text_active1);
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ImU32 col = GetColorU32(is_displaying_hint ? c::text::text1 : c::text::text_active);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        ImVec2 backup_pos = window->DC.CursorPos;
        ItemSize(flags & ImGuiInputTextFlags_NoLabel ? ImVec2(size_arg.x, size_arg.y) : ImVec2(size_arg.x, size_arg.y + 25), style.FramePadding.y);
        if (!ItemAdd(total_bb, id, &frame_bb, ImGuiItemFlags_Inputable))
        {
            EndGroup();
            return false;
        ...
        PushStyleVar(ImGuiStyleVar_ChildBorderSize, style.FrameBorderSize);
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0)); // Ensure no clip rect so mouse hover can reach FramePadding edges
        bool child_visible = BeginChildEx(false, label, "o", id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove);
    ...
    {
        // Support for internal ImGuiInputTextFlags_MergedItem flag, which could be redesigned as an ItemFlags if needed (with test performed in ItemAdd)
    ItemSize(ImRect(total_bb.Min, total_bb.Max + ImVec2(0, flags & ImGuiInputTextFlags_NoLabel ? 0:25)));
        if (!(flags & ImGuiInputTextFlags_MergedItem))
            if (!ItemAdd(total_bb, id, &frame_bb, ImGuiItemFlags_Inputable))
                return false;
        item_status_flags = g.LastItemData.StatusFlags;
  ...
  //  {
   ...
   // }
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::accent,0.45f));
                }
                rect_pos.x = draw_pos.x - draw_scroll.x;
                rect_pos.y += g.FontSize;
            }
    ...
    RenderTextColor(window->DrawList->_Data->Font, ImVec2(frame_bb.Max.x - size_arg.x, frame_bb.Min.y - 23), ImVec2(FLT_MAX, FLT_MAX), GetColorU32(it_anim->second.text), label, ImVec2(0, 0));
...
bool ImGui::NewInputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        PushStyleVar(ImGuiStyleVar_ChildBorderSize, style.FrameBorderSize);
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0)); // Ensure no clip rect so mouse hover can reach FramePadding edges
        bool child_visible = NewBeginChildEx(label, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
    ...
    const bool hovered = NewItemHoverable(frame_bb, id);
    if (hovered)
        g.MouseCursor = ImGuiMouseCursor_TextInput;
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            unsigned int c = '\t'; // Insert TAB
            if (NewInputTextFilterCharacter(&c, flags, callback, callback_user_data, ImGuiInputSource_Keyboard))
                state->OnKeyPressed((int)c);
        }
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, theme_a::inputtext_color[0], anim_speed);
    it_anim->second.main_color = ImLerp(it_anim->second.main_color, theme_a::inputtext_color[1], anim_speed);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, theme_a::inputtext_color[2], anim_speed);
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                for (const char* s = clipboard; *s; )
                {
                    ...
                    if (c == 0)
                        break;
                    if (!NewInputTextFilterCharacter(&c, flags, callback, callback_user_data, ImGuiInputSource_Clipboard))
                        continue;
                    clipboard_filtered[clipboard_filtered_len++] = (ImWchar)c;
                }
                clipboard_filtered[clipboard_filtered_len] = 0;
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    if (BeginChild(false, "undopoints", "o", ImVec2(0.0f, GetTextLineHeight() * 15), true)) // Visualize undo state
    {
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    */
    ...
    draw_list->AddCircleFilled(ImVec2(state.hue, picker_pos.y + 212), 5, GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
    draw_list->AddCircle(ImVec2(state.hue, picker_pos.y + 212), 5, GetColorU32(c::checkbox::outline));
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddCircleFilled(ImVec2(state.alpha, picker_pos.y + 235), 5, GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
        draw_list->AddCircle(ImVec2(state.alpha, picker_pos.y + 235), 5, GetColorU32(c::checkbox::outline));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect clickable(rect.Min + ImVec2(width - 20, 0), rect.Max-ImVec2(0,1));
    ItemSize(ImRect(clickable.Min, clickable.Max));
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(col_rgb), c::picker::rounding);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.mark = ImLerp(it_anim->second.mark, selected ? ImColor(c::accent) : ImColor(0,0,0,0), g.IO.DeltaTime * 6.f);
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(text_min+ImVec2(0,-3), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
...
bool ImGui::SliderFloat1(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, bool search_enable)
{
    if (search_enable)
    {
        std::string labelstr = label;
...
void ImGui::CheckboxWishTips(const char* label, bool* v, const char* text)
{
    ImGui::Checkbox(label, v);
    ImGui::SetCursorPosY(GetCursorPosY() - 10);
    ImGui::TextColoredWishFont(font::calibri_bold_hint, c::text::text, text);
...
bool ImGui::ColorEdit5(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(c::bg::background, 0.8f));
        PushStyleColor(ImGuiCol_Border, GetColorU32(c::bg::outline));
        PushStyleColor(ImGuiCol_WindowShadow, GetColorU32(c::accent, 0.3f));
        PushStyleVar(ImGuiStyleVar_PopupRounding, c::bg::rounding);
        ...
        if (BeginPopup("picker"))
        {
            ...
            if (ImGui::Button("Copy", ImVec2(109, 30)))
                memcpy(custom2.col_buf, col, components * sizeof(float));

            ImGui::SameLine(0, 9.0f);

            if (ImGui::Button("Paste", ImVec2(109, 30))) {

                memcpy(col, custom2.col_buf, components * sizeof(float));
                value_changed = true;
            }
            ...
            if (g.CurrentWindow->BeginCount == 1)
            {
                for (int a = 0; a <= 11; ++a) {
                    float ratio = a / 11.0f;
                    ImColor color(1.0f - ratio, 0.2f - ratio, 0.0f);

                    std::string button_name = "ColorButtonA" + std::to_string(a);
                    if (ImGui::color_button(button_name.c_str(), ImVec2(19, 20), color)) {
                        col[0] = color.Value.x;
                        col[1] = color.Value.y;
                        col[2] = color.Value.z;
                        col[3] = 1.0f;
                    ...
                    if ((a + 1) % 12 != 0) ImGui::SameLine(0, 0.f);
                }

                for (int b = 0; b <= 11; ++b) {
                    float ratio = b / 11.0f;
                    ImColor color(0.2f, 0.8f - ratio, 0.3f - ratio);

                    std::string button_name = "ColorButtonB" + std::to_string(b);
                    if (ImGui::color_button(button_name.c_str(), ImVec2(19, 20), color)) {
                        col[0] = color.Value.x;
                        col[1] = color.Value.y;
                        col[2] = color.Value.z;
                    ...
                    if ((b + 1) % 12 != 0) ImGui::SameLine(0, 0.f);
                }

                for (int c = 0; c <= 11; ++c) {
                    float ratio = c / 11.0f;
                    ImColor color(0.5f, 0.6f, 1.0f - ratio);

                    std::string button_name = "ColorButtonC" + std::to_string(c);
                    if (ImGui::color_button(button_name.c_str(), ImVec2(19, 20), color)) {
                        col[0] = color.Value.x;
                        col[1] = color.Value.y;
                        col[2] = color.Value.z;
                    ...
                    if ((c + 1) % 12 != 0) ImGui::SameLine(0, 0.f);
                }

                for (int d = 0; d <= 10; ++d) {
                    float ratio = d / 10.0f;
                    ImColor color(0.1f, 0.6f - ratio, 1.0f - ratio);

                    std::string button_name = "ColorButtonD" + std::to_string(d);
                    if (ImGui::color_button(button_name.c_str(), ImVec2(19, 20), color)) {
                        col[0] = color.Value.x;
                        col[1] = color.Value.y;
                        col[2] = color.Value.z;
                    ...
                    if ((d + 1) % 12 != 0) ImGui::SameLine(0, 0.f);

                    if (d == 10)
                        ImGui::colorpicker_button(font::icomoon_menu, "Color Picker", "q", ImVec2(19, 20), &openpicker);
                ...
                if (openpicker)
                {
                    ...
                    ImGuiColorEditFlags picker_flags_to_forward = ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_ | ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaBar;
                    ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf;
                    SetNextItemWidth(square_sz * 10.4f); // Use 256 + bar sizes?
                    value_changed |= ColorPicker4("##picker", col, picker_flags, &g.ColorPickerRef.x);
                }
                ...
                if (ImGui::Button("+", ImVec2(30, 20)))
                {
                    ...
                    if (add_status) {
                        ...
                        if (color_x.size() >= 12) {
                            color_x.erase(color_x.begin());
                            color_y.erase(color_y.begin());
                            color_z.erase(color_z.begin());
                            color_a.erase(color_a.begin());
                        }
                ...
                if (InputTextEx("##v", "HEX COLOR", buf, IM_ARRAYSIZE(buf), ImVec2(150, 20), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase | ImGuiInputTextFlags_NoLabel))
                {
                    value_changed = true;
                    char* p = buf;
                ...
                if (!openpicker)
                {
                    ...
                    SetNextItemWidth(227);
                    SliderFloat_popup("##Alpha", &col[3], 0.0f, 1.0f, "%.2f");
                ...
                for (int i = 0; i < color_x.size(); i++) {
                    std::string button_name = "ColorButton" + std::to_string(i);
                    if (ImGui::color_butto2n(button_name.c_str(), ImVec2(19, 20), ImColor(color_x[i], color_y[i], color_z[i], color_a[i]), 1)) {
                        col[0] = color_x[i];
                        col[1] = color_y[i];
                        col[2] = color_z[i];
                        col[3] = color_a[i];
                    }
                    if ((i + 1) % 12 != 0) ImGui::SameLine(0, 0.f);
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    GetWindowDrawList()->AddText(font::icomoon_menu, 14.f, text_pos - ImVec2(30, -4), GetColorU32(c::text::text_active), icon);

    GetWindowDrawList()->AddText(font::calibri_bold, 10.f, text_pos - ImVec2(0, -4), GetColorU32(c::text::text_active), label);
    if (menu_is_open) {

        GetWindowDrawList()->AddText(font::icomoon_menu, 14.f, pos + ImVec2(offsets->OffsetMark + extra_w + 6, 4), GetColorU32(c::text::text_active), "l");
...
THIS IMGUI MENU UI BY TG:(@AN1XER)
