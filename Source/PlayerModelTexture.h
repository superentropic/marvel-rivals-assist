...
#include "playermoder_tex.h"

#pragma comment(lib, "windowscodecs.lib")

namespace PlayerModelTexture {

    static constexpr UINT SRV_SLOT = 66;
    ...
    static bool DecodeFromMemory(const unsigned char* data, size_t dataSize,
                                  ...
                                  UINT& outW, UINT& outH)
    {
        ...
        if (FAILED(hr)) { conv->Release(); frame->Release(); decoder->Release(); stream->Release(); factory->Release(); return false; }
                     ...
                     ID3D12CommandQueue* cmdQueue)
    {
        ...
        std::vector<uint8_t> pixels;
        UINT w = 0, h = 0;
        if (DecodeFromMemory(playermoder_size, sizeof(playermoder_size), pixels, w, h)) {
